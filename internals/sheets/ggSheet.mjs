...
const outputFolder = 'src/i18n/lang';
...
function jsonToSheetData(tabName) {
  ensureFileSync(`${outputFolder}/${DEFAULT_LOCALE}.json`);
  const defaultLanguageJson = readJsonSync(
    `${outputFolder}/${DEFAULT_LOCALE}.json`
  );
  const mapData = Object.keys(defaultLanguageJson).reduce((acc, key) => {
    acc[key] = LANGUAGES.reduce((prev, lang) => {
      if (lang.code === DEFAULT_LOCALE) {
        ...
        ensureFileSync(`${outputFolder}/${lang.code}.json`);
        prev[lang.code] =
          readJsonSync(`${outputFolder}/${lang.code}.json`)[key] || '';
      }
      return prev;
    }, {});
    return acc;
...
async function push(id, tabIndex = 0) {
  try {
    ...
    const { title, sheetId } = spreadsheetData.sheets[tabIndex].properties;
...
async function pull(id, tabIndex = 0) {
  try {
    execSync('pnpm i18n:extract');
    let spreadsheetData = config;
    let spreadsheetId = id || config.spreadsheetId;
    ...
    if (spreadsheetId) {
      ...
      const { title } = spreadsheetData.sheets[tabIndex].properties;
      ...
      for (const lang of LANGUAGES) {
        ensureFileSync(`${outputFolder}/${lang.code}.json`);
        const localLang = await readJsonSync(
          `${outputFolder}/${lang.code}.json`
        );
        const newLang = { ...localLang, ...languagesJson[lang.code] };
        await outputJsonSync(`${outputFolder}/${lang.code}.json`, newLang);
      }
